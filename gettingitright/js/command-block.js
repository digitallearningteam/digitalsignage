...
function textHandler($content) {
    ...
    $('.PlayerContent').append('<div style="' + styles + ' position: absolute; background-color: ' + bColor + '; display: none;" data-transition-type="' + type +
        '" data-transition-delay="' + delay + '" data-transition-period="' + period + '" class="TextContent ContentBlock ckeditor_content">' + htmlUnescape(text) + '</div>');
...
function overlayvideoHandler($content, blockNo) {
    ...
    var poster = $content.find(mediaType).attr('posterSrc');
    ...
    currentPage.find('OverlayScene').each(function () {
        ...
        var TextColour = $(this).attr('TextColour');
        var Opcacity = $(this).attr('Opcacity');
        var Text = $(this).text();

        var scene = "<div id=\"Overlay_" +
            Id + "\" " +
            "Style=\"background:" + hexToRgbA(BackgroundColour, Opcacity) + "; " +
            "color:" + TextColour + "; \"" +
            "class=\"VideoOverlay " +
            Template + "\" " +
            "data-item-id=\"" +
    ...
    //Edge and safari can play m3u8's for legacy videos
    var mobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    if ((mobile || (!mobile && (/Edge/i.test(navigator.userAgent) || (/Safari/i.test(navigator.userAgent) && !/Chrome/i.test(navigator.userAgent))))) && legacy == "True") {
    ...
    styles = '<style>.vjs-control-bar { background: linear-gradient(to right,' + defaults.HeaderColour1 + ',' + defaults.HeaderColour2 + '); }' +
        '.vjs-load-progress div, .vjs-play-progress { background: ' + defaults.NavColour1 + ' !important; }' +
        '.vjs-play-progress:before { color: ' + defaults.NavColour1 + ' !important; }</style > ';

    html = '<div style="' + blockStyles + '">' + scenes + '<video id="' + playerId + '" class="video-js" preload="auto" ' + poster + '>' +
...
function videoHandler($content, blockNo) {
    ...
    if (mediaType == "video") {
        ...
        html = '<div style="' + blockStyles + '"><video id="video_player_' + blockNo + '" class="video-js" preload="auto" ' + poster + '>' +
            mpd + '<source src="' + src + '.m3u8' + '" type="application/x-mpegURL"></source><source src="' + src + '.' + fileType + '" type="video/mp4"></source>' +
        ...
        html = '<div style="' + blockStyles + '"><audio id="video_player_' + blockNo + '" class="video-js" preload="auto" ' + poster + '>' +
...
function interactionHandler($content, contentNo) {
    ...
    $('#iframe_' + contentNo).load(function () {
        ...
        //Apply theme colours
        $('#iframe_' + contentNo).contents().find('head').append('<style type="text/css">' + interactionStyles + '.accent-color-text { color: ' + defaults.NavColour1 + ' !important; } .accent-color-border { border-color: ' + defaults.NavColour1 + ' !important; } .accent-color-background { background-color: ' + defaults.NavColour1 + ' !important; }</style>');
        $('#iframe_' + contentNo).contents().find('.ckeditor_interaction').addClass('ckeditor_content');
...
function assessmentHandler($block) {
    ...
    var textBlock = '<div style="' + styles + 'background-color: rgba(255,255,255,0); overflow:hidden; display: none;" data-transition-type="' + $block.attr('transition') +
        '" data-transition-delay="' + $block.attr('transitionDelay') + '" data-transition-period="' + $block.attr('transitionPeriod') + '" ' +
        'class="TextContent ContentBlock">' + htmlUnescape($block.find('text').text()) + '</div>';
    ...
    var pageContentAssessment = "<div>" +
        ...
        "<div class=\"AssessmentScoreContainer animated jackInTheBox delay-1s\" style=\"float:left\">" +
        "<p class=\"AssessmentScoreTitle\">Your Score</p>" +
        "<p id=\"AssessmentScoreAchieved\" class=\"AssessmentScore\">" + score + "%</p>" +
        "</div>" +
        "<div class=\"AssessmentScoreContainer animated jackInTheBox \" style=\"float:right\">" +
